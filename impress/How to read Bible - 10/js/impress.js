...
(function (document) {
	...
	var addToolbarButton = function (toolbar) {
		var html =
			'<button id="impress-autoplay-playpause" ' + // jshint ignore:line
			'title="Autoplay" class="impress-autoplay">' + // jshint ignore:line
			getButtonText() +
			"</button>"; // jshint ignore:line
		...
		util.triggerEvent(toolbar, "impress:toolbar:appendChild", { group: 10, element: toolbarButton });
	};
})(document);
...
(function (document) {
	...
	document.addEventListener(
		...
		function (event) {
			...
			util.triggerEvent(document, "impress:help:add", { command: "F5 / ESC", text: "Fullscreen: Enter / Exit", row: 200 });
		},
		false
	);
})(document);
...
(function (document, window) {
	...
	var impressConsole = (window.impressConsole = function (rootId) {
		...
		// Return the object
		allConsoles[rootId] = { init: init, open: open, clockTick: clockTick, registerKeyEvent: registerKeyEvent, _init: _init };
		return allConsoles[rootId];
	});
...
(function (document) {
	...
	document.addEventListener(
		...
		function (event) {
			...
			util.triggerEvent(document, "impress:help:add", { command: "Left &amp; Right", text: "Previous &amp; Next step", row: 1 });
		},
		false
	);
})(document);
...
(function (document, window) {
	...
	document.addEventListener(
		...
		function (event) {
			...
			api.lib.gc.addEventListener(
				window,
				"resize",
				api.lib.util.throttle(function () {
	...
	var substep = function (event) {
		...
		if (event.detail.reason === "prev") {
			...
			if (el) {
				...
				triggerEvent(step, "impress:substep:leave", { reason: "prev", substep: el });
